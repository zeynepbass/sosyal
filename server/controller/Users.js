...
const signup=async (req,res)=>{
    ...
    try {
        ...
        const result=await User.create({email,password:sifrelenmisParola,name:`${firstName} ${lastName}`});

        const token=jwt.sign({email:result.email,id:result._id},'aos-secret-key',{expiresIn:'3h'})
